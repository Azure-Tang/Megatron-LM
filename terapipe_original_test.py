...
from megatron.model import GPTModelTerapipe, GPTModel
...
class TestTransformerLayers(unittest.TestCase):
    ...
    def test_output_consistency(self, mock_validate_args, mock_parse_args):
        ...
        layer = model[0].module.module.language_model.encoder.layers[0]

        seq = args.seq_length # 128
        # 准备输入数据
        input_seq = torch.randn((seq, 4, args.hidden_size), device='cuda', dtype=torch.float16)
        torch.save(input_seq, 'tmp_result_input.pt')

        # 生成上三角掩码矩阵
        attention_mask = torch.tril(torch.ones((1,1,seq,seq), device=input_seq.device))

        # forward
        output = layer(input_seq, attention_mask=attention_mask)

        # 存储输出
        torch.save(output, 'tmp_result_output.pt')
